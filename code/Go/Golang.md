...
[channel 实现原理，底层实现结构](https://golang.design/go-questions/channel/struct/)
...
1. 停止信号
2. 与 timer 结合，一般有两种玩法：实现超时控制，实现定期执行某个任务。
3. 解耦生产方和消费方，成为两者的数据通道
4. 控制并发数
...
Go 的 map 是基于哈希表实现的。哈希表是一种数据结构，可以用来快速查找键值对。它通过将键映射到哈希值，然后将哈希值映射到特定的索引位置，以加速对值的查找。
...
context 主要用来在 goroutine 之间**传递上下文信息**，包括：取消信号、超时时间、截止时间、k-v 等。

#### context 有什么作用

在 一组 goroutine 之间传递共享的值、取消信号、deadline……
...
Flame Graph（火焰图）是一种用于可视化性能分析数据的强大工具，它以直观的方式展示了调用栈的深度和开销。Flame Graph 的基本特点包括：

1. **横向展示调用栈**：Flame Graph 沿着水平轴展示调用栈，从左到右显示调用链。每个条块代表一个函数调用，条块的宽度表示该函数占用的 CPU 时间比例。
    
2. **颜色表示深度**：通常，Flame Graph 中使用颜色来表示不同的函数，深色区块代表耗时较多的函数。颜色的选择通常是随机的，不会反映实际性能问题。
    
3. **垂直对齐**：如果两个条块在垂直方向对齐，意味着它们是同一个调用链上的不同函数。
    
4. **展示开销**：Flame Graph 的高度代表了调用栈的深度，而宽度表示了开销。因此，您可以轻松地看出哪些函数占用了最多的 CPU 时间。
