...
4. 解析结果返回：权威域名服务器接收到DNS解析器的请求后，会返回该域名对应的IP地址。DNS解析器将这个IP地址缓存起来，并将解析结果返回给浏览器。
    
5. 建立连接：浏览器拿到解析结果后，会使用HTTP协议与Web服务器建立连接。浏览器发送一个HTTP请求，包含请求的方式（GET、POST等）、请求的资源路径、请求头等信息。
