...
3. 为了保证事务的 ACID 特性，数据库管理系统（DBMS）采取了一些机制和策略：
	- **事务日志**：DBMS 将每个事务的操作记录到事务日志中。如果系统崩溃，可以通过回放事务日志来恢复数据库到正确的状态。
	- **并发控制**：隔离性的实现涉及并发事务的管理，例如锁定机制和多版本并发控制（MVCC）。
	- **缓冲区管理**：持久性通过将数据缓冲在内存中，并在合适的时候写入磁盘，以确保数据不会因为系统崩溃而丢失。
	- **回滚段**：为了保证原子性和一致性，DBMS 使用回滚段来存储事务的中间状态，以便在事务回滚时恢复数据。
