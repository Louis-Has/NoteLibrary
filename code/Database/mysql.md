...
1. 唯一性要求:
    - 唯一索引要求索引列的值在整个表中必须是唯一的，但可以包含空值（NULL）。也就是说，唯一索引可以允许一个空值，但不允许重复的非空值。
    - 主键索引也要求索引列的值在整个表中必须是唯一的，但与唯一索引不同，主键索引不允许包含任何空值。主键是表的唯一标识符，每一行的**主键值都必须唯一且非空**。
2. 数量限制:
    - **一张表可以包含多个唯一索引，但只能有一个主键索引**。主键索引用于唯一标识每一行，而唯一索引用于确保列值的唯一性。
    - 主键索引自带唯一性和非空性的约束，而唯一索引只提供唯一性约束，但允许包含一个空值。
3. 自动增长属性:
    - **主键索引通常与自动增长（Auto-Increment）列一起使用**，这允许数据库为每个新插入的行生成唯一的主键值。
    - 唯一索引没有自动增长属性，不会自动为索引列生成唯一值。
4. 物理存储:
    - **主键索引通常是表的物理存储顺序**，它决定了数据在磁盘上的排列方式。
    - 唯一索引不一定会影响数据的物理存储顺序。
...
1. **InnoDB：** InnoDB 是 MySQL 的默认存储引擎，它支持事务、行级锁定、外键约束等高级特性。这使得它特别适用于事务处理和具有高并发读写需求的应用程序。InnoDB 提供了可靠的数据完整性和崩溃恢复。
    
2. **MyISAM：** MyISAM 是另一个常见的存储引擎，它适用于读密集型应用，但不支持事务和行级锁。MyISAM 在读取速度上很快，但不适合要求数据完整性和事务支持的应用。
    
3. **MEMORY：** MEMORY 存储引擎将表数据存储在内存中，适用于需要非常快速读取的情况，但不适合大规模数据或需要持久性的数据。它对于缓存和临时存储非常有用。
    
4. **NDB（也称为 Cluster）：** NDB 存储引擎是 MySQL Cluster 的一部分，它提供了高可用性、分布式数据库解决方案。它适用于需要自动分区和水平扩展的应用。
    
5. **Archive：** Archive 存储引擎用于高度压缩的只读数据。它对于存档和大规模历史数据存储非常有用。
    
6. **TokuDB：** TokuDB 存储引擎是一个事务性存储引擎，专注于高性能和压缩。它特别适用于需要高写入速度和大规模数据的应用。
    
7. **Aria：** Aria 存储引擎是 InnoDB 的替代品，具有类似的特性，但更轻量，适用于嵌入式系统和小型数据库。
    
8. **FederatedX：** FederatedX 存储引擎允许在不同的 MySQL 服务器之间查询和检索数据，而无需复制数据。
    

选择正确的存储引擎取决于您的应用程序需求。如果需要事务支持和数据完整性，通常选择 InnoDB。如果需要快速读取且可以容忍一些数据风险，可以考虑 MyISAM。
...
6. [数据库常见知识点总结-最左前缀匹配原则、EXPLAIN 命令详解](https://www.cnblogs.com/xuwc/p/14007766.html)
